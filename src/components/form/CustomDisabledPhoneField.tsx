...
const CustomDisabledPhoneField: FC<FormCustomDisabledPhoneFieldProps> = ({ label = '', code, number, noLabel = false }): ReactElement => {
    return (
        <FormControl mt={4}>
            ...
            <HStack spacing={2}>
                <Field as={Input} name={code} type="text" placeholder="Indice" size='lg' w="30%" disabled />

                <Field as={Input} name={number} type="text" placeholder="Numéro" size='lg' w="70%" disabled />
            </HStack>
        </FormControl>
    );
};
